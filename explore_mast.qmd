...
-   <div>
    ...
    mastitis_data_final <- treatment_data %>%
      select(id_animal, event, date_event, protocols_remaining_after_numbers1)

    # Merge exit dates into the original dataset
    mastitis_data_final <- mastitis_data_final %>%
      left_join(exit_cows, by = "id_animal")%>%  # Add date_exit for cows that have left
        mutate(
        days_until_exit = as.numeric(date_exit - date_event),  # Compute days until exit
        event_occurred = ifelse(!is.na(date_exit), 1, 0)  # 1 = exited, 0 = still in herd
      )

    # # Fit Kaplan-Meier survival model
    #fitKM <- survfit(Surv(days_until_exit, event_occurred) ~ protocol_label, data = treatment_data)
