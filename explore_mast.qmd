...
treatment_data <- events_formatted %>%
  filter(event %in% c("MAST", "CULTURE"), (str_starts(protocols_remaining_after_numbers1, "AHV") |
...
# Identify animals treated twice within 14 days
 treatment_data <- treatment_data %>%
  arrange(id_animal, date_event, protocols_remaining_after_numbers1) %>% 
  group_by(id_animal) %>%
  mutate(
   prev_event_date = lag(date_event), #previous date is the entry prior to date_event, looks by animal because of group_by
    ...
    retreated_within_14_days = ifelse(between(days_since_last_event, 14, 21), TRUE, FALSE)
   #rewrote to count as retreatment if after 14 days since initial MAST event but within 21 days. 
  ) %>%
  ungroup()

# Create a new dataset with only retreatments as a data check
...
total_events <- treatment_data %>%
  count(protocols_remaining_after_numbers1, name = "total_events")

# Count retreatments (events that occurred within 14 - 21 days)
retreatment_counts <- treatment_data %>%
  filter(retreated_within_14_days == TRUE) %>%
  count(protocols_remaining_after_numbers1, name = "retreatment_events")

# Merge total events with retreatments
retreatment_data <- left_join(retreatment_counts, total_events, by = "protocols_remaining_after_numbers1") %>%
  mutate(retreatment_percentage = (retreatment_events / total_events) * 100)

# Plot retreatment percentage per protocol
ggplot(retreatment_data, aes(x = protocols_remaining_after_numbers1, 
                             y = retreatment_percentage, 
                             fill = protocols_remaining_after_numbers1)) +
  geom_bar(stat = "identity", width = 0.5, color = "black") +
   geom_text(aes(label = paste0(round(retreatment_percentage, 1), "%")),  # Add percentage labels
            vjust = -0.5, size = 3, fontface = "bold") +  # Adjust label position & size
   scale_fill_manual(values = c("AHVmastitis" = "purple", "SpectraMastLC" = "pink"))+
  theme_minimal() +
  labs(
    title = "Retreatments as % of Total Treatments by Protocol",
