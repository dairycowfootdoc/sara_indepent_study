...
# Assign the most recent fresh date to each repro event
repro_data <- repro_data %>%
  filter(event %in% c("BRED", "PREG", "OPEN")) %>%  # Keep only repro events
  left_join(fresh_data, by = "id_animal") %>%       # Merge fresh dates with repro events
  filter(FDAT <= date_event) %>%                    # Keep only fresh dates before repro event
  group_by(id_animal, date_event) %>%               # Group by animal and repro event date
  slice_max(FDAT, n = 1, with_ties = FALSE) %>%     # Select the most recent fresh date
  ungroup() %>%
  rename(date_fresh = FDAT)                         # Rename column for clarity
...
eligible_cows_data <- lapply(seq_along(periods[-length(periods)]), function(i) {
  ...
  eligible_cows <- repro_data %>%
    filter(
      date_fresh <= (period_next - 50),   # FDAT must be at least 50 days before next period
      event %in% c("BRED", "OPEN")  # Must have BRED or OPEN status
    ) %>%
    summarise(eligible_cows = n())
  data.frame(
    period_start = period_start,
    eligible_cows = eligible_cows$eligible_cows
  )
}) %>%
bind_rows()
  
# Calculate # preg: Count if min event_date (preg) is in period = current period preg #
