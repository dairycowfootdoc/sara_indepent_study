...
# Pre-filter events to only include relevant events before each period's end date
events_filtered <- events_formatted %>%
  filter(event %in% c("PREG", "ABORT"))

# Create a function to find the latest PREG and ABORT events before each period_end_date
get_latest_preg_abort_events <- function(repro_data, events_filtered) {
  repro_data %>%
    # Perform a left join to bring in all repro_data rows
    left_join(
      events_filtered %>%
        ...
        summarise(
          ...
          .groups = "drop"  # Ensure it returns a dataframe without the group structure
        ) %>%
        mutate(
    ...
    # Keep only events before the period_end_date
    left_join(
      repro_data %>%
    ...
    filter(latest_preg_event <= period_end_date | is.na(latest_preg_event)) %>%
    filter(latest_abort_event <= period_end_date | is.na(latest_abort_event))
}

# Apply the function to repro_data
