...
repro_data <- events_formatted %>%
  ...
  select(id_animal, id_animal_lact, date_birth, lact_number, event, date_event, dim_event, R, date_repro_dx)
...
repro_data <- repro_data %>%
  mutate(
    period_start = findInterval(date_event, periods, rightmost.closed = TRUE),
    period_start_date = periods[pmax(period_start, 1)],  # Assign start date
...
# Add VWP date (50 days after fresh date)
repro_data <- repro_data %>%
  mutate(VWP_date = as.Date(date_fresh) + vwp)


#       *Eligibility Logic:*

# Pre-filter events to only include PREG and ABORT events
events_filtered <- events_formatted %>%
  filter(event %in% c("PREG", "ABORT"))

# Function to get the latest PREG and ABORT event before each period_end_date
get_latest_preg_abort_events <- function(repro_data, events_filtered) {
  # Find the most recent PREG and ABORT event per id_animal_lact and period_end_date
...
# Apply function to pull latest pregnancy and abortion events
repro_data <- get_latest_preg_abort_events(repro_data, events_filtered)

# Determine pregnancy status based on the latest events
repro_data <- repro_data %>%
  mutate(
    pregnant_status = case_when(
      !is.na(latest_abort_event) & latest_abort_event > latest_preg_event & latest_abort_event <= period_start_date ~ "Not Pregnant",
      !is.na(date_fresh) & date_fresh > latest_preg_event & date_fresh <= period_start_date ~ "Not Pregnant",  
      !is.na(latest_preg_event) & (is.na(latest_abort_event) | latest_abort_event < latest_preg_event) & 
        (is.na(date_fresh) | date_fresh < latest_preg_event) ~ "Pregnant",
      TRUE ~ "Not Pregnant"
    )
  )

count_active_by_period <- function(repro_data) {
  # Filter to get only cows eligible to be bred
  bred_eligible_data <- repro_data %>%
    filter(
      !is.na(date_fresh),  # Ensure fresh date exists
      VWP_date <= period_end_date, # Ensure passed VWP
      is.na(date_archive) | date_archive >= period_start_date, # Ensure alive
      pregnant_status == "Not Pregnant"  # Exclude pregnant cows
    ) %>%
    distinct(id_animal_lact, period_start_date, lact_number, .keep_all = TRUE)  # Ensure one row per cow per period

  # Count cows per lactation group within each period
  active_counts <- bred_eligible_data %>%
    group_by(period_start_date, lact_number) %>%
    summarise(cows_active = n(), .groups = "drop") %>%
    # Pivot lactation groups into columns
    pivot_wider(names_from = lact_number, values_from = cows_active, values_fill = 0) %>%
    # Add total cows per period
    mutate(total_period = rowSums(select(., -period_start_date), na.rm = TRUE)) %>%
    arrange(period_start_date)  # Ensure chronological order

  return(list(active_counts = active_counts, bred_eligible_data = bred_eligible_data))
}

# Apply function and store outputs
results <- count_active_by_period(repro_data)
active_counts_all_periods <- results$active_counts
eligible_data <- results$bred_eligible_data
