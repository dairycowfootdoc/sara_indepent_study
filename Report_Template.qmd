...
repro_data <- events_formatted %>%
  ...
  select(id_animal, id_animal_lact, date_birth, lact_number, event, date_event, dim_event, date_repro_dx)
...
count_active_by_period <- function(repro_data) {
  repro_data %>%
    # Keep only cows eligible to be bred
    filter(
...
active_counts_all_periods <- count_active_by_period(repro_data)
