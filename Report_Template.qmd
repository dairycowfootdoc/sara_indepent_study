...
# use negative 21 to get it to start with date_end and rev to make biggest number most recent
  ...
  left_join(
    latest_events %>%
      select(id_animal_lact, period_number, 
             ...
             date_concieved),  # Only keep the relevant columns
...
# There are duplicates in things like ABORT, BRED PREG occur in same time period for example 5970
# are cows that abort pregnant before so count as numerator for a bit or just get back tracked to open so not counted in retro calc? 
...
# this tags breds and also deals with problems due to multiple events so I think afte this we can do unique to remove duplicate times
...
eligibility_long <- eligible_cows_gc %>% 
  ...
  summarise(total_eligible = n(), .groups = "drop")  # Count pregnancies per period and lactation group
...
animals_repro_gc4 <- animals_repro_gc3 %>% 
  arrange(id_animal_lact, period_start_date)
...
pregnant_counts_by_period <- animals_repro_gc4 %>%
...
# Reproduction and THI

```{r}

# preg_risk_plot_data <- preg_risk_long %>%
#   filter(lact_number == 1, period_start_date >= date_start, period_start_date <= date_end)
# 
# lowest_points <- preg_risk_plot_data %>%
#   filter(pregnancy_risk == min(pregnancy_risk, na.rm = TRUE))
# 
# preg_risk_plot <- ggplot(preg_risk_plot_data, aes(x = period_start_date, y = pregnancy_risk)) +
...
#   theme_minimal()
# 
# # Create a separate plot for the THI data
# thi_plot <- ggplot() + 
#   # Plot the THI data with color-changing line segments
#   geom_segment(data = weather_weekly, 
#                aes(x = date_weather, y = avg_THI, xend = next_date, yend = next_THI, color = next_THI_color), 
...
#   # Add titles and labels
#   labs(
#     title = "Thermal Heat Index (THI) Over Time",
#     subtitle = "Weekly Thermal Heat Index and Color-coded Segments",
#     x = "Date",
#     y = "THI",
#     caption = "Data Source: VisualCrossing API"
#   ) +
...
#   ) +
#   
#   # Set the y-axis for THI values
#   scale_y_continuous(
#     name = "THI"
#   ) +  # No secondary axis needed for just THI
#   
#   theme(
#     legend.position = "top"  # Position the legend at the top
#   )
# 
# combined_plot <- preg_risk_plot + thi_plot + plot_layout(ncol = 1)  # Stack plots vertically, or use nrow = 1 for horizontal
# 
