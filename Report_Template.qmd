...
repro_data <- repro_data %>%
  ...
  #   # Count the cows in each lactation group for this period
  #   active_counts <- active_animals %>%
  #     group_by(lact_number) %>%
  #     summarise(cows_active = n(), .groups = "drop") %>%
  #     spread(key = lact_number, value = cows_active, fill = 0)
  ...
  #   active_counts$period_start_date <- period_start_date
  #   
  #   # Add a total count column by summing across lactation groups
  #   active_counts$total_period <- rowSums(active_counts[, -which(names(active_counts) == "period_start_date")], na.rm = TRUE)
  #   
  #   # Append the counts for this period to the final results
  #   active_counts_all_periods <- bind_rows(active_counts_all_periods, active_counts)
  # 
  # return(active_counts_all_periods)
