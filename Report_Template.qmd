...
repro_data <- events_formatted %>%
  ...
  select(id_animal, date_birth, lact_number, event, date_event, dim_event, date_repro_dx)
...
filtered_animals <- animal_lactations |>
  filter(date_fresh <= date_end) |>  # Filter cows that freshened before date_end
  mutate(year_fresh = year(date_fresh)) |>  # Find freshening year
  filter(lact_number > 0) |>  # Keep only lactating cows
  filter(
    # Cows that freshened before start date but still active
    (date_fresh <= date_start & (date_archive_max >= date_start | is.na(date_archive_max))) |
    # Cows that freshened after start date but before end
    (date_fresh >= date_start & date_archive_max <= date_end) |
    (date_fresh >= date_start & is.na(date_archive_max))  # Keep animals with no archive date
  ) 
...
total_cows <- filtered_animals |> 
  ...
  select(year_fresh, cows_total)  # Remove lact_groups column

# Summarize by lactation group (lact_number)
summary_lact <- filtered_animals |> 
  group_by(year_fresh, lact_number) |> 
  summarise(cows_total = n(), .groups = "drop") |>  # Count cows in each lactation group
  ungroup() |> 
...
count_active_by_month <- function(month, filtered_animals) {
  ...
  active_animals <- filtered_animals |> 
    filter(
      ...
      (date_fresh <= month_end & (is.na(date_archive_max) | date_archive_max >= month_start))
    )
...
```{r}
# Remove 'total_monthly' column from the original dataset
monthly_data_cleaned <- monthly_data |> 
  select(-total_monthly)  # Remove the total_monthly column
...
ggplot(combined_data, aes(x = month, y = cows_active, color = lactation, group = lactation)) +
  ...
  geom_line(size = 1.2) +  
  # Add points for each data location
  ...
  theme(
    ...
    legend.title = element_text(size = 12),  # Slightly larger legend title
    legend.text = element_text(size = 10),  # Adjust legend text size for clarity
    panel.grid.major = element_line(color = "gray90", size = 0.5),  # Lighter gridlines
    panel.grid.minor = element_blank()  # Remove minor gridlines to keep it clean
  )
...
# Pull Fresh dates:
fresh_data <- events_formatted %>%
  filter(event == "FRESH") %>%
  select(id_animal, FDAT = date_event)

# Assign time periods to repro data
repro_data <- repro_data %>% 
  mutate(
    period_start = findInterval(date_event, periods, rightmost.closed = TRUE),
    period_start_date = periods[period_start]  # Assign the start date of the period
  )

# Keep only the most recent fresh event per cow
fresh_data_recent <- fresh_data %>%
  group_by(id_animal) %>%
  arrange(FDAT) %>%  
  slice_tail(n = 1) %>%   # Take only the most recent fresh event per cow
  ungroup()

# Merge fresh dates with repro events
repro_data <- repro_data %>%
  left_join(fresh_data_recent, by = "id_animal") %>%  # Merge with most recent fresh event
  filter(lact_number > 0) %>%
  group_by(id_animal, date_event) %>%
  mutate(
    date_fresh = max(FDAT[FDAT <= date_event], na.rm = TRUE)  # Most recent fresh date before repro event
  ) %>%
  ungroup()

# Assign the closest future fresh date if no past fresh event exists
repro_data <- repro_data %>%
  group_by(id_animal, date_event) %>%
  mutate(
    date_fresh = ifelse(is.infinite(date_fresh), min(FDAT[FDAT > date_event], na.rm = TRUE), date_fresh)
  ) %>%
  ungroup()

# Convert Inf values back to NA (for cows with no fresh date at all)
repro_data <- repro_data %>%
  mutate(date_fresh = ifelse(is.infinite(date_fresh), NA, date_fresh))

# # Calculate eligible for breeding: if FDAT < next period start date - 50 AND RC = OPEN or BRED = eligible for next period

# Create a data frame to store eligible cows per period
eligible_cows_data <- lapply(seq_along(periods[-length(periods)]), function(i) {
  period_start <- periods[i]
  period_next <- periods[i + 1]  # Next period start date

  # Filter eligible cows for this period
  eligible_cows <- repro_data %>%
    filter(
      date_fresh <= (period_next - vwp),   # FDAT must be at least 50 days before next period
      !event %in% c("PREG", "ABORT")       # Exclude cows that are pregnant or aborted
    ) %>%
    distinct(id_animal, period_start_date) %>%  # Ensure each cow is counted only once per period
    summarise(eligible_cows = n())

  data.frame(
    period_start = period_start,
    eligible_cows = ifelse(nrow(eligible_cows) > 0, eligible_cows$eligible_cows, 0)  # Handle empty cases
  )
}) %>%
bind_rows()
...
# Count unique bred events per period for each cow
# bred_count <- bred_data %>%
#   group_by(id_animal, period_start) %>%
#   summarise(unique_bred_events = n_distinct(date_event), .groups = "drop")

# Count unique pregnancy events per period for each cow
# preg_count <- preg_data %>%
#   group_by(id_animal, period_start) %>%
#   summarise(unique_preg_events = n_distinct(date_event), .groups = "drop")

# Combine both counts and calculate the 21-day pregnancy risk
# preg_risk_data <- bred_count %>%
#   left_join(preg_count, by = c("id_animal", "period_start")) %>%
#   mutate(
#     preg_risk = ifelse(unique_bred_events > 0, unique_preg_events / unique_bred_events, NA)
#   )
