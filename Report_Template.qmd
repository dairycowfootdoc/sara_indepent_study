...
if (!is.null(weather_data_raw)) {
  ...
  thi_plot <- ggplot(weather_data, aes(x = date_weather, y = THI)) +
...
last_year <- Sys.Date() - years(1)  # Get the date for one year ago
milk_weekly_last_year <- milk_weekly %>%
  filter(week >= last_year)
...
combined_plot <- thi_plot + 
  geom_line(data = milk_weekly_last_year, aes(x = week, y = avg_milk), color = "steelblue", size = 1) +  # Add milk production line
  scale_y_continuous(
    limits = c(min(c(min(weather_data$THI), min(milk_weekly_last_year$avg_milk))),
               max(c(max(weather_data$THI), max(milk_weekly_last_year$avg_milk)))),
