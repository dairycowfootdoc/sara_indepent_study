...
animals_repro_test <- animals_repro %>%
  # to get rid of cow that are before VWP
  # keeping them screws the lag below
  filter(vwp_status == 1) |> 
  group_by(id_animal_lact) %>%
  arrange(id_animal_lact, period_start_date) %>%
  mutate(
    # Set eligibility_status to 1 only when vwp_status == 1 and pregnant_status == 0
    # Set to 1 if both conditions are met
    eligibility_status = case_when(
      # event == "DNB" ~ 0,
      # !is.na(date_archive) & 
      #   date_archive >= period_start_date
      # & date_archive <= period_end_date ~ 0,
      vwp_status == 1 & pregnant_status == 0 ~ 1,
...
eligible_cows <- animals_repro_test %>% 
>>>>>>> bfc7f58b23d314438f0852176402a6151c2f5399
