...
animals_repro <- animal_lactations %>% 
  ...
  rename(date_fresh_animal = date_fresh)
...
animals_repro <- all_periods %>%
  ...
  fill(date_fresh_animal, .direction = "downup") %>%  # Ensure lact info is copied
  ungroup()
...
# Pull Fresh dates:
fresh_data <- animal_lactations %>%
  select(id_animal_lact, date_fresh)

# Merge fresh dates with repro events by id_animal_lact 
repro_data <- repro_data %>%
  left_join(fresh_data, by = "id_animal_lact")

# Calulcate vwp
repro_data <- repro_data %>% 
  mutate(date_vwp = as.Date(date_fresh)+vwp)
  
# set up event data 
events_filtered <- events_formatted %>%
  filter(event %in% c("PREG", "ABORT"))

# Get the latest PREG and ABORT event before each period_end_date
latest_events <- events_filtered %>%
  # Join period_end_date from repro_data to determine valid events
  left_join(
    repro_data %>%
      select(id_animal_lact, period_end_date) %>%
      distinct(),
    by = "id_animal_lact"
   ) %>%
  # Keep only events that happened BEFORE each period_end_date
  filter(date_event <= period_end_date) %>%
  group_by(id_animal_lact, period_end_date) %>%
  summarise(
    latest_preg_event = max(ifelse(event == "PREG", date_event, NA), na.rm = TRUE),
    latest_abort_event = max(ifelse(event == "ABORT", date_event, NA), na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
...
# Merge latest PREG and ABORT event dates into repro_data
repro_data <- repro_data %>%
  left_join(latest_events, by = c("id_animal_lact", "period_end_date"))
...
repro_data <- repro_data %>%
  mutate(
    pregnant_status = case_when(
      ...
      !is.na(latest_preg_event) & (is.na(latest_abort_event) | latest_abort_event < latest_preg_event) & 
...
repro_data <- repro_data %>% 
  ...
  mutate(bred_status = ifelse(any(event == "BRED"), 1, 0)) %>%
  ungroup()
...
repro_data <- repro_data %>% 
   mutate(
    vwp_status = case_when(
      is.na(date_fresh) ~ 1,  # If date_fresh is NA, set vwp_status to 1
      date_vwp < period_end_date ~ 1,  # If date_vwp is before period_end_date, set to 1
...
animals_repro <- animals_repro %>%
  left_join(
    repro_data %>% 
      select(id_animal_lact, period_start_date, event, R, date_fresh, date_vwp, 
             period_number, period_end_date, bred_status, pregnant_status,   vwp_status), 
    by = c("id_animal_lact", "period_start_date", "period_end_date", "period_number")  
  )

# assing VWP date and VWP status to animal data with no events
animals_repro <- animals_repro %>%
  mutate(
    date_vwp = ifelse(is.na(date_vwp), as.Date(date_fresh_animal) + vwp, date_vwp),
    date_vwp = as.Date(date_vwp),  # Ensure it's stored as a Date type
    vwp_status = case_when(
      is.na(vwp_status) & is.na(date_fresh_animal) ~ 1,  # If vwp_status is NA and date_fresh_animal is NA, set to 1
      is.na(vwp_status) & date_vwp < period_end_date ~ 1, # If vwp_status is NA and date_vwp < period_end_date, set to 1
      is.na(vwp_status) ~ 0,  # If vwp_status is NA and doesn't meet previous conditions, set to 0
      TRUE ~ vwp_status  # Keep existing values
    )
  )

# get rid of archived cows
animals_repro <- animals_repro %>%
  filter(date_archive >= date_start | is.na(date_archive))

# get rid of animals with no fresh date (lact 32)
animals_repro <- animals_repro %>%
  filter(!is.na(date_fresh_animal))
...
# Set bred status to 0 for NA
animals_repro <- animals_repro %>% 
  mutate(
    bred_status = ifelse(is.na(bred_status), 0, bred_status),
    pregnant_status = ifelse(is.na(pregnant_status), 0, pregnant_status)
  )

# convert binary to numeric for later use
animals_repro <- animals_repro %>%
  mutate(
    bred_status = as.numeric(bred_status),  # Convert to numeric
    pregnant_status = as.numeric(pregnant_status),  # Convert to numeric
    vwp_status = as.numeric(vwp_status)  # Convert to numeric
  )
...
animals_repro <- animals_repro %>%
  ...
  arrange(id_animal_lact, period_start_date) %>%
  mutate(
    # Set eligibility_status to 1 only when vwp_status == 1 and pregnant_status == 0
    eligibility_status = case_when(
      vwp_status == 1 & pregnant_status == 0 ~ 1,  # Set to 1 if both conditions are met
      TRUE ~ 0  # Otherwise, set to 0 by default
    )
  ) %>%
  mutate(
    # For rows with a DNB event or date_archive within the period, set eligibility_status to 0
    eligibility_status = case_when(
      event == "DNB" ~ 0,  # If event is DNB, set eligibility_status to 0
      !is.na(date_archive) & date_archive >= period_start_date & date_archive <= period_end_date ~ 0,  # If date_archive is within the period, set eligibility_status to 0
      TRUE ~ eligibility_status  # Otherwise, retain the previous eligibility_status value
    )
  ) %>%
  mutate(
    ...
    eligibility_status = if_else(
      lag(eligibility_status, default = 1) == 0, 0, eligibility_status  # If previous row has eligibility_status = 0, set current to 0
    )
  ) %>%
  ungroup()

#Create eligibility data set that only has one entry per cow per period with eligibiity status. If 0 in period, entry is 0, if all 1's then entry is 1
eligible_cows <- animals_repro %>% 
  filter(!is.na(eligibility_status)) %>%
...
# need number of bred_events in repro_data that resulted in pregnancy, dynamically add

# Filter eligible cows to only include bred events

# bred_eligible_data <- eligible_data %>% 
#   filter(event == "BRED")
# 
# # Count the number of pregnant cows per period and lactation group
# pregnant_counts_by_period <- bred_eligible_data %>%
#   filter(R %in% c("P", "A")) %>%  # Keep only rows where R is "P"
#   group_by(period_start, lact_number) %>%  # Group by period and lactation group
#   summarise(pregnant_count = n(), .groups = "drop") %>%  # Count occurrences
#   pivot_wider(names_from = lact_number, values_from = pregnant_count, values_fill = 0) %>%  # Pivot lactation groups into columns
#   mutate(total_pregnant = rowSums(select(., -period_start), na.rm = TRUE)) %>%  # Add total pregnant count per period
